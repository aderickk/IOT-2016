...
class bmp183Dal:
	def add(self, var_temp, var_pressure, var_sea_pressure, var_altitude):
		...
		new_id = _bmp183_table.insert_one(new_data).inserted_id
		if not new_id:
			return false
		else:
			return new_id
	...
	def read_All(self):
		result = _bmp183_table.find()
		return result

	def read_By_Date(self, var_req_date):
		result = _bmp183_table.find({"date": var_req_date})
		return result
