...
class tsl2591Dal:
	def add(self, var_ir_luminosity, var_full_luminosity, var_lux):
		...
		new_id = _tsl2591_table.insert_one(new_data).inserted_id
		if not new_id:
			return false
		else:
			return new_id
	...
	def read_All(self):
		result = _tsl2591_table.find()
		return result

	def read_By_Date(self, var_req_date):
		result = _tsl2591_table.find({"date": var_req_date})
		return result
